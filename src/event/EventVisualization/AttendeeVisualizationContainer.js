...
class AttendeeVisualizationContainer extends Component {
  ...
  getCx = () => {
    if (this._isQuadrantOne()) {
      ...
      return (
        Math.sin(angleDelta * DEGREES_TO_RADIANS) * this.getDistance() + (this.props.boundingHeight / 2)
      );
      ...
      return (
        (this.props.boundingHeight / 2) - Math.sin(angleDelta * DEGREES_TO_RADIANS) * this.getDistance()
      );
  ...
  getCy = () => {
    if (this._isQuadrantOne()) {
      const angleDelta = this.state.alpha;
      return (
        Math.cos(angleDelta * DEGREES_TO_RADIANS) * this.getDistance() + (this.props.boundingHeight / 2)
      );
    } else if (this._isQuadrantTwo()) {
      const angleDelta = 180 - this.state.alpha;
      return (
        (this.props.boundingHeight / 2) - Math.cos(angleDelta * DEGREES_TO_RADIANS) * this.getDistance()
      );
    } else if (this._isQuadrantThree()) {
      const angleDelta = this.state.alpha - 180;
      return (
